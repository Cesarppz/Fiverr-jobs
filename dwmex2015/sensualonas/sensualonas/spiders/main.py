...
main_url = 'https://sensualonas.com/'
...
class Webscrape(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        if input_category == 'todas' and input_geozone == 'todas':
            ...
            url = f'{main_url}/escort-{input_geozone}/'
        elif input_geozone == 'todas' and input_category != 'todas':
            url = f'{main_url}/escort-{input_category}/'
        # else:
    ...
    def parse(self, response):
        ...
        if url == main_url:
            links = set(response.xpath('//div[@class="category-listing"]/a[@class="single-link"]/@href').getall())
            for idx, link in enumerate(links):
                logger.info(f'Category {idx} / {len(links)}')
                yield response.follow(link, callback=self.parse)

        else:
            links = set(response.xpath('//div[@class="individual-listing"]/a[@class="single-link"]/@href').getall())
    ...
    def new_parse(self, response, **kwargs):
        ...
        title = response.xpath('//h1[@class="escort_name"]/text()').get().capitalize()
        try:
            geo_zone = response.xpath('//li[@class="li_caracteristicas"]/*[contains(text(),"Estado:")]/following-sibling::a/text()').get().strip()
        except:
            geo_zone = None
        #Categoria
        category =  geo_zone
        #Description
        try:
            description = self.remove_spaces(' '.join(response.xpath('//div[@class="descripcion-escort"]/p//text()').getall()).strip().capitalize())
        except:
            description = None
        phone =  response.xpath('//h2[@class="list_whatsapp_escort"]/text()').get()
        try:
            whatsapp = response.xpath('//a[@class="contacto-whatsapp"]/@href').get()
        except:
            whatsapp = None
        email = None
        ...
        yield{
            ...
            'Nombre de la Página':'Sensualonas'
            }
